/* Vercel serverless function — serves the base task list, gated by login.
 *
 * This list contains real names/emails/phone numbers, so it must NEVER be
 * readable before authentication. It used to ship as the public static file
 * todo-data.js; that file is no longer deployed (see .vercelignore). The app
 * fetches this endpoint AFTER sign-in with the user's Supabase access token in
 * the Authorization header; we verify that token against Supabase and only then
 * return the data. Anything missing/invalid/expired -> 401 with no data
 * (fail closed).
 *
 * The task content is unchanged from the old todo-data.js, so existing
 * task_overrides (keyed by "sectionId::index") still line up.
 */
...
module.exports = async (req, res) => {
  ...
  // Fail closed: no config or no token -> no data.
  if (!url || !anon || !token) {
    res.status(401).json({ error: "unauthorized" });
    return;
  }

  // Verify the token with Supabase. /auth/v1/user returns the user only for a
  // valid, unexpired *user* access token. We require an actual user id in the
  // response so the public anon key (which is a JWT too, but has no user) can't
  // be replayed here as a bearer token.
  try {
